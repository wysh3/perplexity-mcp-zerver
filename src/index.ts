...
class PerplexityMCPServer {
    ...
    try {
      ...
      for (const waitUntil of waitUntilOptions) {
        ...
        try {
          ...
          logError(`Navigation with waitUntil: ${waitUntil} failed:`, navError)
      ...
      if (!pageUrl.includes('perplexity.ai')) {
        safeLog(`Unexpected URL: ${pageUrl}`)
      ...
      if (!searchInput) {
        safeLog('Search input not found, taking screenshot for debugging')
  ...
  private async waitForSearchInput(
    ...
    for (const selector of possibleSelectors) {
      try {
        ...
        logError(`Selector '${selector}' not found or not interactive`)
      }
    }
    ...
    safeLog('No working search input found')
    return null
  }
  ...
  private async retryOperation<T>(
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (i === maxRetries - 1) {
          safeLog(`Maximum retry attempts (${maxRetries}) reached. Giving up.`)
          break
        }
  ...
  private async performSearch(query: string): Promise<string> {
    ...
    const operationTimeout = setTimeout(() => {
      safeLog('Global operation timeout reached, initiating recovery...')
      ...
      return await this.retryOperation(async () => {
        ...
        for (const proseSelector of proseSelectors) {
          try {
            ...
            logError(`Selector ${proseSelector} not found, trying next...`)
          }
        }
